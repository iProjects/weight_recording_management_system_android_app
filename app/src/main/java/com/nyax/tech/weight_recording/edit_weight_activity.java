...
public class edit_weight_activity extends AppCompatActivity {

    private final static String TAG = edit_weight_activity.class.getSimpleName();
    ArrayList<notificationdto> _lstnotificationdto = new ArrayList<notificationdto>();

    Button btnweightslist;
    Button btn_update_weight;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.edit_weight_layout);
        ...
        btn_update_weight = (Button) findViewById(R.id.btn_update_weight);
        ...
        btn_update_weight.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                try {
                    ...
                    if (TextUtils.isEmpty(txt_weight_weight.getText().toString())) {
                        log_info_messages(Utils.format_error_spannable_string("weight cannot be null.").toString());
                        Log.e(TAG, "weight cannot be null.");
                        is_valid = false;
                    }
                    if (TextUtils.isEmpty(txt_weight_date.getText().toString())) {
                        log_info_messages(Utils.format_error_spannable_string("date cannot be null.").toString());
                        Log.e(TAG, "date cannot be null.");
                        is_valid = false;
                    }
                    if (!is_valid) {
                        return;
                    ...
                    new update_record_in_Background_Async_Task().execute(_weight_ui_dto);
        ...
        log_info_messages("finished edit_weight_activity initialization.");
    ...
    private class update_record_in_Background_Async_Task extends AsyncTask<weight_ui_dto, Void, String> {
        ...
        protected void onPreExecute() {
            ...
            Log.e(TAG, "updating record in datastore...");
            log_info_messages("updating record in datastore...");
        ...
        protected String doInBackground(weight_ui_dto... _weight_ui_dto) {
            ...
            try {
                db = new sqlite_database_helper_utilz(getApplicationContext());
                boolean is_update_successful = db.update_weight(_weight_ui_dto[0]);

                if (is_update_successful)
                    return "record successfully persisted in datastore.";
                else
                    return "record saving failed.";
            } catch (Exception ex) {
                Log.e(TAG, ex.toString());
                return ex.toString();
            }
        ...
        protected void onPostExecute(String msg) {
            ...
            final Intent list_weights_activity = new Intent(getApplicationContext(), list_weights_activity.class);
            startActivity(list_weights_activity);
